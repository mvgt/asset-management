...
public class ReportCategoryElect extends HttpServlet {
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		Conn c = new Conn();
		java.sql.Connection con = c.getConnection();
		try {
			Statement stmt = con.createStatement();
			ResultSet rs=stmt.executeQuery("select distinct(title), subtype from itemtable where type='electronics' order by subtype");
			...
			out.println("<h2 align='center'><b>STOCK(ELECTRONICS)</b></h2>");
			out.println("<br>");
